            ...
            <div id="checkout-form-container">
                ...
                <form id="checkout-form">
                    <div class="form-row" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group"><label>Full Name</label><input type="text" id="ship-name" required placeholder="John Doe"></div>
                        <div class="form-group"><label>Email Address</label><input type="email" id="ship-email" required placeholder="john@example.com"></div>
                    </div>
                    <div class="form-group"><label>Shipping Address</label><textarea id="ship-address" required placeholder="123 Street Name, Area, City - PinCode" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 8px; font-family: inherit;"></textarea></div>
                    <div class="form-row" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group"><label>Phone Number</label><input type="tel" id="ship-phone" required placeholder="+91 XXXXX XXXXX"></div>
                        <div class="form-group"><label>Payment Method</label><select id="payment-method" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 8px;"><option value="cod">Cash on Delivery</option><option value="card">Credit / Debit Card</option><option value="upi">UPI / Net Banking</option></select></div>
                    </div>
                    <div id="card-details" style="display: none; margin-top: 15px; padding: 15px; background: #f9f9f9; border-radius: 8px;"><input type="text" placeholder="Card Number" style="margin-bottom: 10px;"><div style="display: flex; gap: 10px;"><input type="text" placeholder="MM/YY"><input type="text" placeholder="CVV"></div></div>
                    <button type="submit" class="btn" style="width: 100%; margin-top: 20px;">Place Order</button>
                </form>
            </div>
            <div id="checkout-success" style="display: none; text-align: center; padding: 40px 0;">
                <div style="width: 80px; height: 80px; background: #e8f5e9; color: #2e7d32; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; font-size: 2.5rem;"><i class="fa-solid fa-check"></i></div>
                <h2>Order Placed Successfully!</h2>
                <p style="color: var(--text-muted); margin: 15px 0 30px;">Thank you for shopping with GlowCare. Your order ID is <span id="order-id" style="font-weight: 600; color: var(--text-color);">#GC-9821</span></p>
                <p>Our agent will process and ship your organic products shortly.</p>
                <button class="btn" onclick="location.reload()" style="margin-top: 30px;">Continue Shopping</button>
            </div>
        </div>
    </div>`;
...
// Ensure the icon works even if script loads before/after DOM
document.addEventListener('click', (e) => {
    if (e.target.id === 'cart-icon' || e.target.closest('#cart-icon')) {
        e.preventDefault();
        openCart();
    }
    if (e.target.id === 'close-cart' || e.target.id === 'drawer-overlay') {
        closeCartDrawer();
    }
});
...
document.addEventListener('submit', (e) => {
            ...
            .then(async res => {
                ...
                if (!res.ok) {
                    if (data.needsVerification === "true") {
                        showToast('Verification Required. Check your email.', 'fa-envelope-circle-check');
                        hideAllAuthForms();
                    ...
                    showToast(`Welcome back, ${data.name}! ✨`, 'fa-circle-check');
                    updateNavbar();
                    setTimeout(() => closeAuthModal(), 1000);
                }
            }).catch(err => {
                console.error('Fetch error:', err);
                showToast('Server unavailable. Please try later.', 'fa-server');
            }).finally(() => {
    ...
    if (target.id === 'register-form') {
        ...
        fetch('/api/customer/register', {
            ...
            if (!res.ok) {
                ...
                showToast('Account created! Plz check email for OTP.', 'fa-paper-plane');
                hideAllAuthForms();
                document.getElementById('otp-email-verify').value = email;
                document.getElementById('otp-form-container').style.display = 'block';
            }
        })
        .catch(err => showToast('Connection problem.', 'fa-wifi'))
        .finally(() => {
            btn.textContent = originalText;
            btn.disabled = false;
        });
    ...
    if (target.id === 'checkout-form') {
        ...
        const orderData = {
            ...
            phoneNumber: document.getElementById('ship-phone').value,
        ...
        .then(async res => {
            if (res.ok) {
                ...
                document.getElementById('order-id').textContent = '#' + savedOrder.orderNumber;
                ...
                const msg = errorData.error || 'Check server logs for database constraint violations';
